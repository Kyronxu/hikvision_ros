...
/*YV12 to BGR format, replace OpenCV's cv::cvtColor*/
void HikvisionCamera::yv12ToBgr(const unsigned char* yv12_data, unsigned char* bgr_data, int width, int height) {
    if (yv12_data == nullptr || bgr_data == nullptr) {
        ROS_ERROR("YV12 to BGR: input/output buffer is null");
        return;
    }
    ...
    const unsigned char* v_data = yv12_data + y_size;
    const unsigned char* u_data = yv12_data + y_size + uv_size;

    for (int i = 0; i < height; ++i) {
        for (int j = 0; j < width; ++j) {
            const int y = static_cast<int>(y_data[i * width + j]);

            const int uv_row = i / 2;
            const int uv_col = j / 2;
            const int uv_idx = uv_row * (width / 2) + uv_col;
            const int v = static_cast<int>(v_data[uv_idx]);
            const int u = static_cast<int>(u_data[uv_idx]);

            //from：https://en.wikipedia.org/wiki/YUV#Conversion_to_RGB
            int b = y + ((359 * (u - 128)) >> 8);  // 1.772 ≈ 359/203（calculate ：1.772*256=454，right move 8 bit）
            int g = y - ((88 * (u - 128) + 183 * (v - 128)) >> 8);  // 0.34414≈88/256，0.71414≈183/256
            int r = y + ((454 * (v - 128)) >> 8);  // 1.402 ≈ 454/256

            b = std::max(0, std::min(255, b));
            g = std::max(0, std::min(255, g));
            r = std::max(0, std::min(255, r));

            const int bgr_idx = (i * width + j) * 3;
            bgr_data[bgr_idx]     = static_cast<unsigned char>(b);
            bgr_data[bgr_idx + 1] = static_cast<unsigned char>(g);
            bgr_data[bgr_idx + 2] = static_cast<unsigned char>(r);
        }
    }
}
...
void HikvisionCamera::decodeCallback(int nPort, char *pBuf, int nSize, FRAME_INFO *pFrameInfo)
{
    ...
    // }
    /* change opencv dependency to manual convert*/
    if (lFrameType == T_YV12) {
        const int curr_width = pFrameInfo->nWidth;
        const int curr_height = pFrameInfo->nHeight;

        sensor_msgs::CameraInfoPtr ci(new sensor_msgs::CameraInfo(camera_info_mgr->getCameraInfo()));
        ci->header.frame_id = frame_id;
        ci->header.stamp = ros::Time::now();
        ci->width = curr_width;
        ci->height = curr_height;

        sensor_msgs::Image msg;
        msg.header = ci->header;
        msg.width = curr_width;
        msg.height = curr_height;
        msg.encoding = "bgr8";
        msg.is_bigendian = 0;
        msg.step = curr_width * 3;
        msg.data.resize(curr_width * curr_height * 3);

        yv12ToBgr(reinterpret_cast<unsigned char*>(pBuf), msg.data.data(), curr_width, curr_height);
        ...
        // frame delay detect by zhaoyu
        if (is_first_frame)
        {
            is_first_frame = false;
            last_frame_header = ci->header;
            this_frame_header = ci->header;
        }
        else
        {
            this_frame_header = ci->header;
            time_difference = (this_frame_header.stamp.toSec() - last_frame_header.stamp.toSec()) * 1000;
            last_frame_header = this_frame_header;
            is_last_frame_delay = is_current_frame_delay;
            if (time_difference >= (1000/frame_rate))
            {
                is_current_frame_delay = true;
                ROS_WARN("[%s] frame time diff too big: %d ms.", camera_name.c_str(), time_difference);
            }
            else
            {
                is_current_frame_delay = false;
                ROS_INFO("[%s] frame time diff: %d ms.", camera_name.c_str(), time_difference);
            }
            
            if(is_last_frame_delay and is_current_frame_delay)
            {
                frame_count += 1;
            }
            else
            ...
            ROS_INFO("[%s] frame_count: %d", camera_name.c_str(), frame_count);

            if((frame_count >= frame_rate) and if_shutdown)
            {
                ROS_ERROR("The system delay is too largr, closing the node ......");
                ros::shutdown();
            }
        }
...
bool HikvisionCamera::initHikSDK()
{
    ...
    data_play_handler = NET_DVR_RealPlay_V40(user_id, &struPlayInfo, dataCallback, this);
...
void HikvisionCamera::initROSIO(ros::NodeHandle& priv_node)
{
    ...
    priv_node.param<int>("frame_rate",frame_rate, 100);
    ROS_INFO("[%s] frame_rate: \t%d", camera_name.c_str(), frame_rate);

    priv_node.param<bool>("if_shutdown",if_shutdown, false);
    ROS_INFO("[%s] if_shutdown: \t%d", camera_name.c_str(), if_shutdown);
