...
void HikvisionCamera::decodeCallback(int nPort, char *pBuf, int nSize, FRAME_INFO *pFrameInfo)
{
    ...
    if (lFrameType == T_YV12)
    {
        ...
        else
        {
            ...
            // ROS_INFO("[%s] frame time difference: \t%d ms.", camera_name.c_str(),time_difference);
            ...
            if (time_difference >= (1000/frame_rate))
            {
                ...
                ROS_WARN("[%s] frame time difference too big(\t%d ms).", camera_name.c_str(), time_difference);
            }
            else
            {
